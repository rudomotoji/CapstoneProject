    ...
    if (event is MedicalShareEventGet) {
      ...
      try {
        final List<MedicalShareDTO> list =
            await healthRecordRepository.getListMedicalShare(
                event.patientId,
                event.medicalInstructionType,
                event.diseaseId,
                event.medicalInstructionIds);
      ...
      try {
        final List<MedInsByDiseaseDTO> list =
            await healthRecordRepository.getAllMedicalToShare(
                event.patientID, event.healthRecordId, event.typeID);
